...
from .data import Batch, Stats, Transition
...
class EnvWrapper(object):
    def __init__(self, env_builder):
        ...
        self.demo = self._underlying.demo
        if len(self.demo) < N_ROLLOUT:
            self.demo += (0,) * (N_ROLLOUT - len(self.demo))
...
def _rollout(
        ...
        envs,
        init_mstate=None,
        act_fn=lambda model, batch, t: model.act(batch)):
    ...
    for t in range(N_ROLLOUT):
        ...
        if FLAGS.gpu:
            batch = batch.cuda()
        act, model_state_ = act_fn(model, batch, t)
...
# TODO dup with above
def _rollout_sup(model1, model2, envs, meta_featurizer):
    feat_envs = [EnvWrapper(lambda: meta_featurizer(e._underlying)) for e in envs]
    bufs1, _, mstate = _rollout(model1, feat_envs)
    bufs2a, _, _ = _rollout(
            model2, envs, mstate,
            lambda model, batch, t: ([e.demo[t] for e in envs], model.mstate()))
    bufs2b, stats, mstate2 = _rollout(model2, envs, mstate)

    assert all(len(buf) == len(env._underlying.demo) for env, buf in zip(envs, bufs2a))

    aug_bufs1 = []
    for i in range(len(bufs2b)):
        last_fr = bufs2b[i][0].forward_reward
        aug_buf = [
            transition._replace(forward_reward=last_fr)
            for transition in bufs1[i]]
        aug_bufs1.append(aug_buf)

    #aug_bufs2 = [[t._replace(forward_reward=1, reward=1) for t in buf] for buf in bufs2a]

    return (aug_bufs1, bufs2a), stats, mstate2
...
def train_sup(model1, model2, train_env_builder, val_env_builder, meta_featurizer, cache_file):
    opt = optim.RMSprop(
        itertools.chain(model1.parameters(), model2.parameters()),
        lr=FLAGS.lr,
        eps=1e-5,
    ...
    def train_helper(batch1, batch2):
        _, mstate1, _ = model1(batch1)
        loss2, _, ll = model2(batch2, mstate1, clone=True)
        loss1, _, _ = model1(batch1, extra_reward=ll)
    ...
    _train(
        lambda envs: _rollout_sup(model1, model2, envs, meta_featurizer),
        train_helper, train_env_builder, val_env_builder, cache_file)
